...
Servo myServo;
int set = 75;
int banDau = set; // Vị trí ban đầu của servo 
int toiThieu = set; // Giới hạn tối thiểu của servo
...
void loop() {
  ...
  else if (digitalRead(3) == LOW && stopButtonPressed == false && banDau <= set) { // Nút dừng
    stopButtonPressed = true;
    tien = false;
    banDau = banDau + 20;
    myServo.write(banDau);
